...
const WinnerAvatar = ({ userId }) => {
  const [showFullAddress, setShowFullAddress] = useState(false);

  // const shortenedAddress = `${userId.slice(0, 6)}...${userId.slice(-4)}`;
  const avatarUrl = `https://api.dicebear.com/6.x/micah/svg?seed=${
    userId || "placeholder"
  }.svg`;
      ...
      {showFullAddress && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl">
            <p className="font-mono text-lg mb-4">{userId}</p>
            <button
