...
const CampaignCard = ({ campaign }) => {
  const rewardIcon = availableRewards.find(reward => reward.name === campaign.reward)?.icon || <Gift className="text-gray-500" />;
  const actionIcon = [...actions.Onchain, ...actions.Offchain].find(action => action.name === campaign.action)?.icon || <Clock className="text-gray-500" />;

  return (
    <Link href={`/`}>
      <div className="bg-gradient-to-br from-white to-indigo-50 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 p-6 mb-4 relative overflow-hidden border-4 border-indigo-50">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-4">
          <Stat icon={<Users className="text-indigo-500" />} label="Participants" value={campaign.participants} />
          <Stat icon={<Award className="text-yellow-500" />} label="Winners" value={campaign.winners} />
          <Stat icon={<Calendar className="text-green-500" />} label="Days Left" value={campaign.daysLeft} />
